...
const PokemonDetailsPage = () => {
  ...
  if (error)
    return (
      <p className="text-center mt-10 text-lg text-red-500">Error: {error}</p>
    );
  if (!pokemon)
  ...
  return (
    <main className="flex flex-col items-center justify-center p-4 md:p-6 lg:p-8 bg-gray-100">
      <div className="w-full max-w-4xl p-6 rounded-lg shadow-md mb-6 custom-overflow min-h-screen h-screen overflow-scroll overflow-x-hidden custom-background">
        <button
          onClick={() => router.push("/")}
          className="mb-6 text-white bg-green-900 hover:bg-[#dc6466] px-4 py-2 rounded-lg"
        >
          Back to Home
        </button>
        <h1 className="text-3xl font-bold capitalize mb-6 text-color text-center">
          {pokemon.name}
        </h1>
        <div
          className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${montserrat.className} antialiased`}
        >
          ...
          <div className="flex flex-col space-y-4 text-color">
